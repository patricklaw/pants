...
class SourceAnalysisTask(Task):
  """A paren Task for buildgen upstream analysis tasks which provide source -> analysis products.
  ...
  def analyze_sources(self, source_relpaths):
    ...
    It is an error if the returned map does not have a key for every passed in source relpath.
    """
    raise NotImplemented
  ...
  def execute(self):
    ...
    with self.invalidated(targets, invalidate_dependents=False) as invalidation_check:
      ...
      if uncalculated_analysis:
        raise Exception(
          '{0} failed to calculate analysis for the following sources:{1}\n'
          .format(
            type(self).__name__,
