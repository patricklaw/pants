...
class SimpleIvyResolve(IvyTaskMixin, JvmToolTaskMixin, Task):
  @property
  def config_section(self):
    return 'simple-ivy-resolve'

  @classmethod
  def product_types(cls):
    return [
      'simple_ivy_classpath',
    ]

  def __init__(self, *args, **kwargs):
    super(SimpleIvyResolve, self).__init__(*args, **kwargs)
  ...
  def execute(self):
    ...
    classpath, _ = self.ivy_resolve(targets, workunit_name='ivy-resolve-the-world')
    products.safe_create_data('simple_ivy_classpath', lambda: classpath)
...
class MapThirdPartyJarSymbols(Task):
  @property
  def config_section(self):
    return 'map-third-party-jar-symbols'
  ...
  def prepare(cls, options, round_manager):
    ...
    round_manager.require_data('scala')
    round_manager.require_data('java')
    round_manager.require_data('simple_ivy_classpath')
  ...
  def execute(self):
    ...
    with self.invalidated(targets, invalidate_dependents=False) as invalidation_check:
      ...
      if invalidation_check.invalid_vts or not os.path.exists(vts_analysis_file):
        classpath = products.get_data('simple_ivy_classpath')
        calculated_analysis = {}
