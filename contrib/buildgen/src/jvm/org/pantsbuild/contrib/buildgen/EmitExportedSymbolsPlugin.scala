...
class EmitExportedSymbolsPlugin(val global: Global) extends Plugin {
  ...
  val outputDir = System.getProperty("org.pantsbuild.contrib.buildgen.exportedSymbolOutputDir")
      ...
      private def traversePackageSymbols(tree: Tree): Seq[String] = {
        tree match {
          ...
          case pd: PackageDef => {
            ...
            Seq(pd.pid.toString) ++ fqChildNames
          }
          ...
          case md: ModuleDef => {
            ...
            md.name.toString match {
              ...
              case other => Seq(md.name.toString) ++ childNames.map(other + "." + _)
            }
          }

          case cd: ClassDef => {
            Seq(cd.name.toString)
          }

          case dd: ValOrDefDef => {
            if (dd.name.toString != "<init>") {
              Seq(dd.name.toString)
            } else {
              Seq.empty
            }
          }
